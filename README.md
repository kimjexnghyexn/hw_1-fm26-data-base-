...
1. 바인드 마운트 경로
```bash
-v ~/fm26/app:... # 홈 디렉토리 기준 절대경로를 하드코딩함
``` 	

해결 : 프로젝트 루트 폴더 안에서 실행한다는 전제로 
```bash
...
2. 호스트 포트 8080	
8080번 포트가 이미 다른 프로그램에 의해 사용 중일 수 있음

해결 : port is already allocated 에러 발생 시 -p 8081:80처럼 다른 호스트 포트로 변경
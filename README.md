...
### 4-1. 작업 디렉토리 확인 및 파일 생성

`pwd`로 현재 위치를 확인하고, `touch`로 선수 데이터 파일을 생성한 뒤 `ls -al`로 생성 여부와 권한을 확인했다.
...
### 4-2. 리다이렉션으로 데이터 입력 (`>` vs `>>`)

`>`(덮어쓰기)로 첫 데이터를 넣고, `>>`(이어쓰기)로 선수를 추가한 뒤 `cat`으로 내용을 검증했다.
